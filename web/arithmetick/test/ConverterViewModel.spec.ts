...
import { allCategoryCards } from "../src/lib/models/CategoryItemModel";
...
describe("Length Conversion Tests", function () {
  let lengthCard = allCategoryCards.find(
    (card) => card.item == CategoryItems.LENGTH
  );
  it("check length outputs", function () {
    expect(lengthCard.title).toBe("Length");
    expect(lengthCard.icon).toBe("fas fa-ruler");
    expect(lengthCard.color).toBe("bg-blue-500");
  });
  it("check length conversions", function () {
    ...
    let inchItem = conversions.find((item) => item.option.unit == Units.Inch);
    expect(inchItem.value).toBeCloseTo(144);
    let mileItem = conversions.find((item) => item.option.unit == Units.Mile);
    viewModel.selectedUnit = mileItem.option;
    conversions = viewModel.getConvertedValues("5");
    let kiloValue = conversions.find(
      (item) => item.option.unit == Units.Kilometer
    );
...
describe("Temperature Conversion Tests", function () {
  ...
  it("check temperature conversions", function () {
    ...
    let celciusItem = conversions.find(
      (item) => item.option.unit == Units.Celsius
    );
...
describe("Mass Conversion Tests", function () {
  let massCard = allCategoryCards.find(
    (card) => card.item == CategoryItems.MASS
  );
  it("check mass outputs", function () {
    expect(massCard.title).toBe("Mass");
    expect(massCard.icon).toBe("fas fa-weight-scale");
    expect(massCard.color).toBe("bg-purple-500");
  });
  it("check mass conversions", function () {
    ...
    let poundItem = conversions.find(
      (item) => item.option.unit == Units.PoundLb
    );
...
describe("Volume Conversion Tests", function () {
  let volumeCard = allCategoryCards.find(
    (card) => card.item == CategoryItems.VOLUME
  );
  it("check volume outputs", function () {
    expect(volumeCard.title).toBe("Volume");
    expect(volumeCard.icon).toBe("fas fa-droplet");
    expect(volumeCard.color).toBe("bg-orange-500");
  });
  it("check volume conversion", function () {
    ...
    let gallonItem = conversions.find(
      (item) => item.option.unit == Units.Gallon
    );
...
describe("Pressure Conversion Tests", function () {
  let pressureCard = allCategoryCards.find(
    (card) => card.item == CategoryItems.PRESSURE
  );
  it("check pressure outputs", function () {
    ...
    expect(pressureCard.color).toBe("bg-emerald-500");
  });
...
describe("Energy Conversion Tests", function () {
  ...
  it("check energy conversion", function () {
    ...
    let kiloItem = conversions.find(
      (item) => item.option.unit == Units.Kilojoule
    );
